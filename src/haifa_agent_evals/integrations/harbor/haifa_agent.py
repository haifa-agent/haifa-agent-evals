...
_JRE_ARCHIVE_PATH = "/tmp/haifa-jre.tar.gz"
_JRE_URL = (
    "https://github.com/adoptium/temurin21-binaries/releases/download/"
    "jdk-21.0.8%2B9/OpenJDK21U-jre_x64_linux_hotspot_21.0.8_9.tar.gz"
)
_JRE_SHA256 = "968c283e104059dae86ea1d670672a80170f27a39529d815843ec9c1f0fa2a03"
...
class HaifaCodingAgent(BaseInstalledAgent):
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        configured_jre = jre_path or os.environ.get("HAIFA_EVAL_JRE_PATH")
        self.jre_path = Path(configured_jre).expanduser().resolve() if configured_jre else None
        ...
        if self.jre_path is not None:
            if not self.jre_path.is_file():
                raise ValueError("Haifa eval JRE archive does not exist")
            if _sha256(self.jre_path) != _JRE_SHA256:
                raise ValueError("Haifa eval JRE archive digest does not match the pinned JRE")
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        await self.exec_as_root(environment, f"install -d -m 0755 {_CONTAINER_ROOT}")
        if self.jre_path is not None:
            await environment.upload_file(self.jre_path, _JRE_ARCHIVE_PATH)
            obtain_jre = "true"
        else:
            obtain_jre = (
                "apt-get update && apt-get install -y curl ca-certificates && "
                f"curl -L --fail --retry 3 -o {_JRE_ARCHIVE_PATH} {_JRE_URL}"
            )
        await self.exec_as_root(
            environment,
            command=(
                "if ! command -v java >/dev/null 2>&1 || "
                "! java -version 2>&1 | grep -q '\"21'; then "
                f"{obtain_jre} && "
                f"echo '{_JRE_SHA256}  {_JRE_ARCHIVE_PATH}' | sha256sum -c - && "
                f"mkdir -p {_CONTAINER_ROOT}/jre && "
                f"tar -xzf {_JRE_ARCHIVE_PATH} -C {_CONTAINER_ROOT}/jre --strip-components=1 && "
                f"rm {_JRE_ARCHIVE_PATH}; fi"
            ),
        ...
        await self.exec_as_root(
            ...
            command=(
                ...
                f"JAVA=$([ -x {_CONTAINER_ROOT}/jre/bin/java ] && "
                f"echo {_CONTAINER_ROOT}/jre/bin/java || command -v java) && "
